"""One-click Colab runner for matched continuous-unit falsification controls."""
...
WORK = Path("/content/sign_unit_controls")
OUTPUT = WORK / "results"
...
def main() -> None:
    ...
    if md5(archive) != SHARD_MD5:
        raise RuntimeError("Shard 1 MD5 mismatch; partial data retained for resume")
    command = [
        sys.executable,
        "-u",
        "-m",
        "sign_semantics.unit_controls",
        "--checkpoint",
        str(checkpoint),
        "--archive",
        str(archive),
    ...
    comparison = (OUTPUT / "control_comparison.csv")
    if not comparison.exists() or comparison.stat().st_size == 0:
        raise RuntimeError("Control comparison was not produced")
    bundle = Path("/content/sign_unit_control_results.zip")
    ...
    archive.unlink()
    print("Raw shard deleted after the complete result bundle was verified.", flush=True)
